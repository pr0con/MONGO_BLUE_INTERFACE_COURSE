...
export function Node() {
	...
	const { rs, request, LCID, AccessToken, databases, collections, setCollections, documents, setDocuments, insertedDocument, setInsertedDocument } = useContext(AppContext);
	
	
	useEffect(() => {
		if(rs === 1) {
			setCurrentOp('Collecting Databases...');
			request({lcid: LCID, access_token: AccessToken}, 'get-databases', null);
		}
	},[rs]);
	...
	useEffect(() => {
		if(database !== '') {
			...
			request({lcid: LCID, access_token: AccessToken}, 'get-collections', database);
		} 
	},[database]);
	...
	useEffect(() => {
		if(collection !== '') {
			...
			request({lcid: LCID, access_token: AccessToken}, 'get-documents', {db: database, col: collection});
	...
	const dropDatabase = () => {
		if(database != '') {
			...
			request({lcid: LCID, access_token: AccessToken}, 'drop-database', database);
	...
	const createDatabase = (e) => {
		if(e.key === 'Enter') {
			...
			request({lcid: LCID, access_token: AccessToken}, 'create-database', newDb);
			setNewDb('');
		}
	}
	...
	const dropCollection = () => {
		...
		request({lcid: LCID, access_token: AccessToken}, 'drop-collection', {db: database, col: collection});
	...
	const createCollection = (e) => {
		if(e.key === 'Enter' && database !== '') {
			...
			request({lcid: LCID, access_token: AccessToken}, 'create-collection', {db: database, col: newCol});
			setNewCol('');
		}
	}
	...
	const saveModifiedDocument = () => {
		if(Object.keys(modifiedDocumentValues).length > 0) {
			if(database !== '' && collection !== '') {
				...
				request({ lcid: LCID, access_token: AccessToken }, 'update-document', {db: database, col: collection, oid: currentDocument._id, values: modifiedDocumentValues });
			}
		}
	}
	...
	const createDocument = () => {
		if(database !== '' && collection !== '') {
			if(!insertMany) {
				...
				request({lcid: LCID, access_token: AccessToken}, 'insert-document', {db: database, col: collection, doc: newDoc});
				setNewDoc({});
			}else if(insertMany) {
				setCurrentOp(`Attempting to Insert Many in -> ${collection} in -> ${database}`);
				request({lcid: LCID, access_token: AccessToken}, 'insert-many', {db: database, col: collection, data: code});
			}
		}
	}
	...
	return(
		<StyledNode className="database-connector">
			<div className="database-connector-title">
				<span className="dct-left">NodeJs</span>
				<span className="dct-center flex-filler"></span>
			...
			<div className="database-connector-content">
				<div className="dcc-left">
					<div className="database-list">
						{ (databases != null && databases.databases.length > 0) && databases.databases.map((db,i) => (
							<div className={`database ${database === db.name ? 'selected' : ''}`} onClick={(e) => setDatabase(db.name)} key={`node-db-${i}`}>
					...
					<div className="collection-list">
						{(collections !== null && collections.length > 0) && collections.map((c,i) => (
							<div className={`collection ${collection === c ? 'selected' : ''}`} onClick={(e) => setCollection(c)} key={`node-collection-${i}`}>
								{c}
							</div>
						))}
					</div>
				...
				<div className="dcc-right">
					...
					<div className="document-list">
						{ (documents !== null && documents.length > 0 && !insertMany) && documents.map((d,i) => (
							<div className={`document ${document === d._id ? 'selected' : ''}`} onClick={(e) => {setDocument(d._id); setCurrentDocument(d);}} key={`node-document-${i}`}>{d._id}</div>
						))}
					</div>
				</div>
			</div>	
