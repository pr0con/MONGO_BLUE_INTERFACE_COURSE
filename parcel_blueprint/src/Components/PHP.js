...
export function PHP() {
	...
	const { rs, wsId, request, databases, collections, setCollections, documents, setDocuments, insertedDocument, setInsertedDocument } = useContext(PHPContext);
	...
	return(
		<StyledPHP className="database-connector">
			<div className="database-connector-title">
				<span className="dct-left">PHP</span>
				...
				<div className="dcc-left">
					<div className="database-list">
						{ (databases != null && databases.length > 0) && databases.map((db,i) => (
							<div className={`database ${database === db ? 'selected' : ''}`} onClick={(e) => setDatabase(db)} key={`php-db-${i}`}>
					...
					<div className="collection-list">
						{(collections !== null && collections.length > 0) && collections.map((c,i) => (
							<div className={`collection ${collection === c ? 'selected' : ''}`} onClick={(e) => setCollection(c)} key={`php-collection-${i}`}>
								{c}
							</div>
						))}
					</div>
				...
				<div className="dcc-right">
					...
					<div className="document-list">
						{ (documents !== null && documents.length > 0 && !insertMany) && documents.map((d,i) => (
							<div className={`document ${document === d._id ? 'selected' : ''}`} onClick={(e) => {setDocument(d._id); setCurrentDocument(d);}} key={`php-document-${i}`}>{d._id}</div>
						))}
					</div>
				</div>
			</div>	
