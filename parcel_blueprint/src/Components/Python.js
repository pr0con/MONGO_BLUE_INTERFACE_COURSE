...
export function Python() {
	...
	const { rs, wsId, request, databases, collections, setCollections, documents, setDocuments, insertedDocument, setInsertedDocument } = useContext(PythonContext);
	...
	useEffect(() => {
		if(collection !== '') {
			...
			request('get-documents', { db: database, col: collection });
	...
	const dropCollection = () => {
		if(database !== '' && collection !== '') {
			...
			request('drop-collection', {db: database, col: collection});
	...
	const createCollection = (e) => {
		if(e.key === 'Enter' && database !== '') {
			...
			request('create-collection', {db: database, col: newCol});
			setNewCol('');
		}
	}
	...
	const saveModifiedDocument = () => {
		if(Object.keys(modifiedDocumentValues).length > 0) {
			if(database !== '' && collection !== '') {
				setCurrentOp(`Updating document ${currentDocument._id.$oid} in -> ${collection} in -> ${database}`);
				request('update-document', {db: database, col: collection, oid: currentDocument._id.$oid, values: modifiedDocumentValues });
			}
		}
	}
	...
	const createDocument = () => {
		if(database !== '' && collection !== '') {
			if(!insertMany) {
				...
				request('insert-document', {db: database, col: collection, doc: newDoc});
				setNewDoc({});
			}else if(insertMany) {
				setCurrentOp(`Attempting to Insert Many in -> ${collection} in -> ${database}`);
				request('insert-many', {db: database, col: collection, data: code});
			}
		}
	}
	
	useEffect(() => {
		if(insertedDocument !== null && '_id' in insertedDocument && '$oid' in insertedDocument._id) {
			setDocument(insertedDocument._id.$oid);
	...
	return(
		<StyledPython className="database-connector">
			<div className="database-connector-title">
				<span className="dct-left">Python</span>
				...
				<div className="dcc-left">
					<div className="database-list">
						{	(databases !== null && databases.length > 0) && databases.map((db,i) => (
							<div className={`database ${database === db.name ? 'selected' : ''}`} onClick={(e) => setDatabase(db.name)} key={`python-db-${i}`}>
					...
					<div className="collection-list">
						{ (collections !== null && collections.length > 0) && collections.map((c,i) => (
							<div className={`collection ${collection === c ? 'selected' : ''}`} onClick={(e) => setCollection(c)} key={`python-collection-${i}`}>
								{c}
							</div>
						))}
					</div>
				...
				<div className="dcc-right">
					...
					<div className="document-list">
						{ (documents !== null && documents.length > 0 && !insertMany) && documents.map((d,i) => (
							<div className={`document ${document === d._id.$oid ? 'selected' : ''}`} onClick={(e) => {setDocument(d._id.$oid); setCurrentDocument(d);}} key={`python-document-${i}`}>
								{d._id.$oid}
							</div>
						))}
					</div>					
